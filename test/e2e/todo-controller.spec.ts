...
describe('todo controller', () =>{
  ...
  it('publish a created todo', async () => {
    ...
    let res = await app.inject({ method: 'POST', url: "/api/todos", payload: newTodo });
    expect(res.statusCode).toEqual(200)

    const todo: Todo = JSON.parse(res.body)
    await mockServer.forPost("/publish/todo").withBody(res.body).thenReply(200, "A mocked response");
    res = await app.inject({ method: 'POST', url: `/api/todos/${todo.id}/publish` });
    expect(res.statusCode).toEqual(200)
  })
})
